...
public:
    Sub(std::string id, std::size_t length, std::size_t num_iter = 1, bool call_shutdown = true)
        : Node("sub" + id, rclcpp::NodeOptions().use_intra_process_comms(true)), _id(id), _first(true), _latency(), _inter_time_arrival(), _call_shutdown(call_shutdown)
        , _matrix_analysis({
            {"latency", _latency.values()},
            {"inter_time_arrival", _inter_time_arrival.values()}
        }), _length{length}, _num_iter(num_iter), count_(0)
    {
        _subscription = this->create_subscription<shc::msg::Data>(
            "data" + std::to_string(length), 1, [this](const shc::msg::Data::UniquePtr msg) {
    ...
    ~Sub()
    {
        _matrix_analysis.dump_values("zc_" + _id + "_size" + std::to_string(_length));
        _matrix_analysis.dump_analysis("zc_" + _id + "_size" + std::to_string(_length));
    }
