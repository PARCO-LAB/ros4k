...
public:
        ...
        }), _length{length}, _num_iter(num_iter), count_(0), _call_shutdown(call_shutdown)
    {
        _subscription = this->create_subscription<shc::msg::Data>(
            "data" + std::to_string(length), 1, [this](const shc::msg::Data::SharedPtr msg) {
    ...
    ~Sub()
    {
        _matrix_analysis.dump_values("iceoryx_" + _id + "_" + std::to_string(_length));
        _matrix_analysis.dump_analysis("iceoryx_" + _id + "_" + std::to_string(_length));
    }
