...
public:
    ...
    {
        _publisher = this->create_publisher<shc::msg::Data>("data" + std::to_string(length), 1);
...
private:
    void timer_callback()
    {
        ...
        auto message = _publisher->borrow_loaned_message();
        init<float>(message.get().arr.data, _length);
        RCLCPP_INFO(this->get_logger(), "Publishing: size %zu", message.get().arr.data.size());
        message.get().header.timestamp = std::chrono::high_resolution_clock::now().time_since_epoch().count();
        message.get().header.frame_id = count_;
