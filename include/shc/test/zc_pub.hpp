...
public:
    Pub(std::string id, std::size_t length, std::size_t num_iter = 1, bool call_shutdown = true)
        : Node("pub"+id, rclcpp::NodeOptions().use_intra_process_comms(true)), count_(0), _length(length), _num_iter(num_iter), _call_shutdown(call_shutdown)
    {
...
private:
    void timer_callback()
    {
        auto message = std::make_unique<shc::msg::Data>();
